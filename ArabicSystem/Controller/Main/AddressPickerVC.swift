...
class AddressPickerVC: UIViewController {
    ...
    @IBOutlet weak var lbl_LocationText: UILabel!
    @IBOutlet weak var btn_DoneText: UIButton!
    ...
    func changeStaticLangauge()
    {
        if LanguageManager.shared.currentLanguage == .en {
            self.lbl_LocationText.text = "Location"
            self.btn_DoneText.setTitle("Done", for: .normal)
        } else {
            self.lbl_LocationText.text = "الموقع"
            self.btn_DoneText.setTitle("تم", for: .normal)
        }
    }
...
//        requestToSearchArea()
    ...
    private func requestToSearchArea()
    {
        var paramDict: [String : AnyObject] = [:]
        paramDict["user_id"] = k.userDefault.value(forKey: k.session.userId) as AnyObject?
        paramDict["lat"] = self.lat as AnyObject
        paramDict["lon"] = self.lon as AnyObject
        
        print(paramDict)
        
        Api.shared.search_ServiceArea(self, paramDict) { responseData in
            if responseData.status == "1" {
                ...
                if LanguageManager.shared.currentLanguage == .en {
                    ...
                    self.alert(alertmessage: responseData.message_ar ?? "")
                }
            }
        }
    }
